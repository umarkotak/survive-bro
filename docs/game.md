...
- The timeline receives only level events whose runtime `show` flag is true. Hidden spawn, durability, and treasure-cadence ramps still execute authoritatively without cluttering the HUD.
...
- Stage 1 Slimes and Slime Kings use the server-owned, near-contact `slime-punch` melee spell. Greater Slimes cast the reusable short-range `enemy-slime-ball`; Phaser generates its red glob texture and brief player-hit flash locally while the server owns targeting, cooldown, range, damage, collision, and removal.
- Authoritative `monster_attacked` events animate Slime Punch as an `85 ms` squash/stretch with a mirrored five-degree lunge and yoyo recovery. Repeated punches reset to the sprite's stable base scale, and active tweens are destroyed with their monster or scene.
...
- The ten-minute `spell-lab` spawns nine cyan all-pool spell chests around the centre. Each rolls three random unowned choices from all six player-available spells until the configurable four-spell inventory is full; later boxes become treasure offers for targeted spell testing.
- Every owned spell is listed in compact clickable cards below the character health panel. The character modal shows character stats only; selecting a spell card opens its authoritative resolved damage, cooldown, range, geometry, cadence, and level details.
